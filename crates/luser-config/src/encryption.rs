...
use crate::{ConfigError, ConfigResult, ConfigSecurityLevel};
...
impl EncryptionManager {
    ...
    pub fn from_base64_key(algorithm: EncryptionAlgorithm, base64_key: &str) -> ConfigResult<Self> {
        let key = STANDARD.decode(base64_key)
            .map_err(|e| ConfigError::EncryptionError(format!("Failed to decode base64 key: {}", e)))?;
    ...
    pub fn from_env(algorithm: EncryptionAlgorithm, env_var: &str) -> ConfigResult<Self> {
        let base64_key = std::env::var(env_var)
            .map_err(|e| ConfigError::EnvError(format!("Failed to read env var {}: {}", env_var, e)))?;
    ...
    pub fn decrypt_string(&self, ciphertext: &str) -> ConfigResult<String> {
        let ciphertext_bytes = STANDARD.decode(ciphertext)
            .map_err(|e| ConfigError::EncryptionError(format!("Failed to decode base64 ciphertext: {}", e)))?;
        
        let plaintext = self.decrypt(&ciphertext_bytes)?;
        String::from_utf8(plaintext)
            .map_err(|e| ConfigError::EncryptionError(format!("Failed to convert decrypted data to string: {}", e)))
    }
    ...
    fn encrypt_aes_gcm(&self, key: &[u8], plaintext: &[u8]) -> ConfigResult<Vec<u8>> {
        ...
        let nonce_bytes = self.nonce_generator.generate_nonce(self.algorithm.nonce_length());
    ...
    fn decrypt_aes_gcm(&self, key: &[u8], ciphertext: &[u8]) -> ConfigResult<Vec<u8>> {
        ...
        if ciphertext.len() < nonce_length {
            return Err(ConfigError::EncryptionError(
                format!("Ciphertext too short: expected at least {}, got {}", 
                        nonce_length, ciphertext.len())
            ));
        }
...
impl ConfigEncryptor {
    ...
    /// 加密配置值
    pub fn encrypt_config_value(&self, key: &str, value: &str, security_level: ConfigSecurityLevel) -> ConfigResult<String> {
    ...
    /// 批量加密配置值
    pub fn encrypt_config_values(&self, values: &[(&str, &str, ConfigSecurityLevel)]) -> ConfigResult<Vec<(String, String)>> {
    ...
    /// 批量解密配置值
    pub fn decrypt_config_values(&self, values: &[(&str, &str)]) -> ConfigResult<Vec<(String, String)>> {
        let mut results = Vec::with_capacity(values.len());
    ...
    fn encrypt_database_config(&self, config: &mut crate::DatabaseConfig) -> ConfigResult<()> {
        ...
        if let Some(cert_path) = &config.ssl_client_key_path {
            if self.is_encrypted_value(cert_path) {
                let decrypted = self.decrypt_config_value("database.ssl_client_key_path", cert_path)?;
                let encrypted = self.encrypt_config_value(
    ...
    /// 加密数据库URL（提取密码部分加密）
    fn encrypt_database_url(&self, url: &str) -> ConfigResult<String> {
        // 解析URL以提取密码部分
        if let Ok(url_obj) = url::Url::parse(url) {
            if let Some(password) = url_obj.password() {
                // 加密密码
                ...
                new_url.set_username(username)
                    .map_err(|_| ConfigError::EncryptionError("Failed to set username".to_string()))?;
                new_url.set_password(Some(&encrypted_password))
                    .map_err(|_| ConfigError::EncryptionError("Failed to set password".to_string()))?;
        ...
        if let Ok(url_obj) = url::Url::parse(url) {
            if let Some(password) = url_obj.password() {
                if self.is_encrypted_value(password) {
                    // 解密密码
                    let decrypted_password = self.decrypt_config_value("database.password", password)?;
                    
                    // 重建URL，用解密后的密码替换加密密码
                    let mut new_url = url_obj.clone();
                    let username = url_obj.username();
                    new_url.set_username(username)
                        .map_err(|_| ConfigError::DecryptionError("Failed to set username".to_string()))?;
                    new_url.set_password(Some(&decrypted_password))
                        .map_err(|_| ConfigError::DecryptionError("Failed to set password".to_string()))?;
    ...
    fn encrypt_redis_url(&self, url: &str) -> ConfigResult<String> {
        ...
        if let Some(captures) = redis_url_pattern.captures(url) {
            ...
            if !password.is_empty() {
                ...
                // 重建URL
                return Ok(format!("redis://{}:{}@{}", username, encrypted_password, rest));
            }
        }
    ...
    fn decrypt_redis_url(&self, url: &str) -> ConfigResult<String> {
        ...
        if let Some(captures) = redis_url_pattern.captures(url) {
            ...
            if !password.is_empty() && self.is_encrypted_value(password) {
                // 解密密码
                let decrypted_password = self.decrypt_config_value("redis.url_password", password)?;
                
                // 重建URL
                return Ok(format!("redis://{}:{}@{}", username, decrypted_password, rest));
            }
        }
    ...
    /// 加密云服务配置
    fn encrypt_cloud_service_config(&self, config: &mut crate::CloudServiceConfig) -> ConfigResult<()> {
        // 加密腾讯云配置
    ...
    /// 解密云服务配置
    fn decrypt_cloud_service_config(&self, config: &mut crate::CloudServiceConfig) -> ConfigResult<()> {
        // 解密腾讯云配置
    ...
    fn decrypt_aliyun_config(&self, config: &mut crate::AliyunCloudConfig) -> ConfigResult<()> {
        ...
        if let Some(access_key_secret) = &config.access_key_secret {
            if self.is_encrypted_value(access_key_secret) {
                let decrypted = self.decrypt_config_value("aliyun.access_key_secret", access_key_secret)?;
    ...
    fn decrypt_aws_config(&self, config: &mut crate::AwsCloudConfig) -> ConfigResult<()> {
        ...
        if let Some(secret_access_key) = &config.secret_access_key {
            if self.is_encrypted_value(secret_access_key) {
                let decrypted = self.decrypt_config_value("aws.secret_access_key", secret_access_key)?;
    ...
    fn decrypt_huawei_config(&self, config: &mut crate::HuaweiCloudConfig) -> ConfigResult<()> {
        ...
        if let Some(secret_access_key) = &config.secret_access_key {
            if self.is_encrypted_value(secret_access_key) {
                let decrypted = self.decrypt_config_value("huawei.secret_access_key", secret_access_key)?;
                config.secret_access_key = Some(decrypted);
            }
        }
    ...
    fn decrypt_storage_config(&self, config: &mut crate::StorageConfig) -> ConfigResult<()> {
        ...
        if let Some(access_key_id) = &config.s3.access_key_id {
            if self.is_encrypted_value(access_key_id) {
                let decrypted = self.decrypt_config_value("storage.s3.access_key_id", access_key_id)?;
        ...
        if let Some(secret_access_key) = &config.s3.secret_access_key {
            if self.is_encrypted_value(secret_access_key) {
                let decrypted = self.decrypt_config_value("storage.s3.secret_access_key", secret_access_key)?;
    ...
    fn decrypt_sms_config(&self, config: &mut crate::SmsConfig) -> ConfigResult<()> {
        if let Some(access_key_secret) = &config.access_key_secret {
            if self.is_encrypted_value(access_key_secret) {
                let decrypted = self.decrypt_config_value("sms.access_key_secret", access_key_secret)?;
                config.access_key_secret = Some(decrypted);
            }
        }
    ...
    fn decrypt_queue_config(&self, config: &mut crate::QueueConfig) -> ConfigResult<()> {
        ...
        if let Some(secret_access_key) = &config.sqs.secret_access_key {
            if self.is_encrypted_value(secret_access_key) {
                let decrypted = self.decrypt_config_value("queue.sqs.secret_access_key", secret_access_key)?;
    ...
    /// 加密扩展配置
    fn encrypt_extensions(&self, extensions: &mut std::collections::HashMap<String, serde_json::Value>) -> ConfigResult<()> {
    ...
    /// 解密扩展配置
    fn decrypt_extensions(&self, extensions: &mut std::collections::HashMap<String, serde_json::Value>) -> ConfigResult<()> {
    ...
    fn parse_encrypted_value(&self, value: &str) -> ConfigResult<(ConfigSecurityLevel, String)> {
        if !self.is_encrypted_value(value) {
            return Err(ConfigError::DecryptionError("Value is not encrypted".to_string()));
        }
        ...
        if parts.len() != 2 {
            return Err(ConfigError::DecryptionError("Invalid encrypted value format".to_string()));
        }
        ...
        let security_level = match security_level_str {
            ...
            _ => return Err(ConfigError::DecryptionError(
                format!("Unknown security level: {}", security_level_str)
            )),
        };
...
/// 便捷函数：加密配置值
pub fn encrypt_config_value(key: &str, value: &str, security_level: ConfigSecurityLevel) -> ConfigResult<String> {
    // 从环境变量获取加密密钥
    let base64_key = std::env::var("GRAPE_ENCRYPTION_KEY")
        .map_err(|e| ConfigError::EnvError(format!("GRAPE_ENCRYPTION_KEY not set: {}", e)))?;
    
    let encryption_manager = EncryptionManager::from_base64_key(EncryptionAlgorithm::Aes256Gcm, &base64_key)?;
...
pub fn init_global_encryptor() -> ConfigResult<()> {
    ...
    if global_encryptor.is_none() {
        // 从环境变量获取加密密钥
        let base64_key = std::env::var("LUSER_ENCRYPTION_KEY")
            .unwrap_or_else(|_| {
                warn!("GRAPE_ENCRYPTION_KEY not set, using default key");
...
impl KeyManager {
    ...
    pub fn new(
        ...
        let manager = Self {
            ...
            key_history: Arc::new(RwLock::new(vec![KeyHistoryEntry {
                ...
                expires_at: chrono::Utc::now() + chrono::Duration::from_std(rotation_interval).unwrap(),
                active: true,
            }])),
            rotation_interval,
        };
    ...
    pub fn rotate_key(&self) -> ConfigResult<String> {
        ...
        *self.current_key.write() = new_key.clone();
        ...
        history.push(KeyHistoryEntry {
            ...
            key: new_key,
            created_at: chrono::Utc::now(),
            expires_at: chrono::Utc::now() + chrono::Duration::from_std(self.rotation_interval).unwrap(),
            active: true,
        });
    ...
    pub fn decrypt_with_historical_keys(
        ...
        match encryption_manager.decrypt(encrypted_data) {
            ...
            Err(_) => {
                ...
                Err(ConfigError::DecryptionError("Failed to decrypt with any historical key".to_string()))
            }
        }
    }
    
    /// 导出密钥历史（加密存储）
    pub fn export_key_history(
        &self,
        export_key: &[u8],
    ) -> ConfigResult<Vec<u8>> {
        let history = self.key_history.read();
        
        let export_data = serde_json::to_vec(&*history)
            .map_err(|e| ConfigError::SerializationFailed(format!("Failed to serialize key history: {}", e)))?;
        
        // 使用导出密钥加密历史数据
        let export_manager = EncryptionManager::new(
            EncryptionAlgorithm::Aes256Gcm,
            export_key.to_vec(),
        )?;
    ...
    pub fn import_key_history(
        ...
        // 解密历史数据
        let import_manager = EncryptionManager::new(
            EncryptionAlgorithm::Aes256Gcm,
            import_key.to_vec(),
        )?;
        ...
        // 反序列化历史数据
        let history: Vec<KeyHistoryEntry> = serde_json::from_slice(&decrypted)
            .map_err(|e| ConfigError::DeserializationFailed(format!("Failed to deserialize key history: {}", e)))?;
